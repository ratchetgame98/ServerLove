...
public class ServerLove extends JavaPlugin{
	...
	public boolean marryPropose;
	public boolean datePropose;
...
public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
	if(commandLabel.equalsIgnoreCase("marry")){
		if(args.length == 0){
			...
			sender.sendMessage(ChatColor.RED + "Available Args: Propose, Accept/Deny, Wedding, CheckPropose, Divorce");
			...
			if(args[0].equalsIgnoreCase("propose")){
				...
				}else if(this.getConfig().getString("Blocked." + sender.getName() + ".block").contains(sender.getName())){
				...
				if(target == null){
					...
					this.getConfig().set("Marry." + sender.getName() + ".married", true);
					this.getConfig().set("Marry." + target.getName() + ".married", true);
				}
			}else if(args[0].equalsIgnoreCase("deny")){
				if(target == null){
					sender.sendMessage(ChatColor.RED + "Please specify your partner");
				...
				marryPropose = getConfig().getBoolean("Marry." + sender.getName() + ".check");
				if(this.getConfig().getBoolean("Marry." + sender.getName() + ".check") == true){
					sender.sendMessage(ChatColor.GREEN + "You have a pending proposal by " + ChatColor.RESET + this.getConfig().getString("Marry." + sender.getName() + ".propose"));
				}else{
					sender.sendMessage(ChatColor.RED + "You have no current pending proposals");
				}
				if(this.getConfig().get("Marry." + sender.getName() + ".propose") == null){
				...
				if(target == null){
					...
					target.sendMessage(ChatColor.RED + "Your partner wants a divorce and you need to confirm the divorce as well. Use " + ChatColor.GOLD + "/marry divorce " + ChatColor.RESET + target.getName() + " confirm " + ChatColor.RED + "to confirm the divorce");
				}
			}
		}if(args.length == 3){
			final Player target = Bukkit.getServer().getPlayer(args[1]);
				...
				if(target == null){
					...
					if(target == null){
						...
						if(args[2].equalsIgnoreCase("confirm")){
							...
							this.getConfig().set("Marry." + sender.getName() + ".married", false);
							this.getConfig().set("Marry." + target.getName() + ".married", false);
						}
					}
				}
			}this.saveConfig();
			...
			if(args.length == 0){
				...
				sender.sendMessage(ChatColor.RED + "Available Args: Block, Update");
			}else if(args.length == 1){
